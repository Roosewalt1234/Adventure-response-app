...
export default function App() {
  ...
  const [view, setView] = useState<'chat' | 'dashboard'>(() => {
    if (typeof window !== 'undefined') {
      return window.location.pathname === '/dashboard' ? 'dashboard' : 'chat';
    }
    return 'chat';
  });
  ...
  useEffect(() => {
    const handlePopState = () => {
      setView(window.location.pathname === '/dashboard' ? 'dashboard' : 'chat');
    };
    window.addEventListener('popstate', handlePopState);
    return () => window.removeEventListener('popstate', handlePopState);
  }, []);

  const handleViewChange = (newView: 'chat' | 'dashboard') => {
    setView(newView);
    const newPath = newView === 'dashboard' ? '/dashboard' : '/';
            ...
            <div className="space-y-2">
              <button 
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${
                  view === 'chat' ? 'bg-emerald-500 text-black font-bold' : 'text-white/60 hover:bg-white/5'
                }`}
              >
              ...
              <button 
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${
                  view === 'dashboard' ? 'bg-emerald-500 text-black font-bold' : 'text-white/60 hover:bg-white/5'
                }`}
              >
        ...
        {isSidebarOpen && (
          <>
            ...
            >
              ...
              <div className="p-6 space-y-8">
                <section>
                  <h2 className="text-xs font-semibold text-white/40 uppercase tracking-wider mb-4">Navigation</h2>
                  <div className="space-y-2">
                    <button 
                      onClick={() => { handleViewChange('chat'); setIsSidebarOpen(false); }}
                      className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${
                        view === 'chat' ? 'bg-emerald-500 text-black font-bold' : 'text-white/60 hover:bg-white/5'
                      }`}
                    >
